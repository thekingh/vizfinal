...
public class parallel_coords extends PApplet {
...
public class ControlPoint {
    ...
    public void applySpringForce(PVector toPos, float len) {
        ...
        // Update force
        f.add(PVector.mult(force, dist * SPRING_CONST / ( (len * 1.0f*NUM_SUBS))));
    }
...
public class Edge {
    ...
    public void applySpringForces() {
        float localSF = SPRING_CONST / (1.0f*NUM_SUBS+1);
...
//        }
        ...
        pushStyle();
        int c = color(0, 0,200, SHOW_ORIGINAL ? 10 : 50);    
        stroke(c);
        strokeWeight(EDGE_WEIGHT);
        if(!BEZLINE) {
            for(int i = 0; i <= NUM_SUBS; i++) {
...
public FDEB_Graph parseCSV(String filename) {
...
class FDEB_Graph 
{
    ...
    public void update(float t) {
        ...
        if(ct == null) {
            generateCT();
            BUNDLE_CONST = GBUNDLE_CONST / (1.0f * edges.size() * NUM_SUBS);
        }
...
int NodeColor  = color(100, 100, 100);
...
float MIN_CONSTRAINT_DISTANCE = 100.0f;
boolean DEBUG_CONSTR_ON = true;
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "parallel_coords" };
    if (passedArgs != null) {
